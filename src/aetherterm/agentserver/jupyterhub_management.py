...
from fastapi import APIRouter, HTTPException, Depends, BackgroundTasks, status, Query, Path
                ...
                try:
                    ...
                    if response.code == 200:
                        ...
                        if server_info.get("ready", False):
                            ...
                            return
                        elif server_info.get("pending") == "spawn":
                            # Still spawning
    ...
    async def stop_user_server(self, server_id: str) -> bool:
        ...
        try:
            ...
            if response.code in [202, 204]:
                ...
                self.log.error(
                    "Failed to stop server", server_id=server_id, status_code=response.code
                )
                return False
