...
class AgentPaneManager:
    ...
    async def create_agent_pane(
        ...
        logger.info(f"エージェントペーンを作成しました: {pane.pane_id} (エージェント: {pane.agent_id})")
    ...
    def register_pane_created_callback(
        self,
        callback: Callable[[AgentPane], None]
    ) -> None:
        """ペーン作成コールバックを登録"""
        if callback not in self._pane_created_callbacks:
            self._pane_created_callbacks.append(callback)
    
    def unregister_pane_created_callback(
        self,
        callback: Callable[[AgentPane], None]
    ) -> None:
        """ペーン作成コールバックを解除"""
    ...
    def register_pane_destroyed_callback(
        self,
    ...
    def unregister_pane_destroyed_callback(
        self,
        callback: Callable[[str], None]
    ) -> None:
        """ペーン破棄コールバックを解除"""
