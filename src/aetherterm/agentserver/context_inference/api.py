...
from .models import OperationType, OperationStage, OperationContext, ContextInferenceResult
...
async def infer_operation_context(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Context inference failed: {str(e)}")
...
async def predict_next_commands(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Command prediction failed: {str(e)}")
...
async def get_active_operations() -> Dict[str, List[OperationContextResponse]]:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Active operations retrieval failed: {str(e)}")


@router.post("/learn-patterns", response_model=PatternLearningResponse)
async def learn_operation_patterns(
    ...
    try:
        ...
        # パターン学習を実行
        learned_patterns = await pattern_learner.learn_patterns_from_history(
            days=request.days
        )
...
async def get_patterns_summary() -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Patterns summary failed: {str(e)}")
