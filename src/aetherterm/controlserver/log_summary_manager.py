...
class LogSummaryManager:
    ...
    async def _generate_periodic_summary(self):
        ...
        logger.info(f"Generated periodic summary: {summary.summary_id} ({summary.event_count} events)")
    ...
    async def _generate_emergency_summary(self, critical_memory: ShortTermMemory):
        ...
        logger.warning(f"Generated emergency summary: {summary.summary_id} (triggered by {critical_memory.severity})")
    ...
    async def _create_summary(
        ...
        # パフォーマンスサマリ
        performance_summary = await self._generate_performance_summary(performance_events) if performance_events else None
        
        # 推奨事項
        recommendations = await self._generate_recommendations(memories, error_events, performance_events)
    ...
    async def _generate_recommendations(
        ...
        if error_events and len(error_events) / len(memories) > 0.1:
            recommendations.append("エラー率が10%を超えています。システムの安定性を確認してください。")
        
        # パフォーマンス問題
        slow_events = [e for e in performance_events if e.metadata.get("response_time", 0) > 1000]
        if slow_events:
            recommendations.append("レスポンス時間が1秒を超えるイベントが検出されました。パフォーマンス最適化を検討してください。")
        ...
        if agent_counts:
            ...
            if max_count > len(memories) * 0.5:
                recommendations.append("特定のエージェントに負荷が集中しています。負荷分散を検討してください。")
