...
class CoordinationDemo:
    ...
    def __init__(self):
        ...
        self.orchestrator.register_intervention_callback(
            self.handle_intervention_with_coordination
        )
    ...
    async def demo_file_conflict(self):
        ...
        # 親エージェントが調整して応答
        response1 = await self.orchestrator.handle_intervention_request(
            agent1_id, intervention1
        )
        logger.info(f"エージェント1への応答: {response1}")
        
        response2 = await self.orchestrator.handle_intervention_request(
            agent2_id, intervention2
        )
    ...
    async def handle_intervention_with_coordination(
        self,
        ...
        if "auth.py" in intervention.message:
            ...
            if "authenticate_user" in intervention.message:
                ...
                logger.info("[親エージェント] ファイルの下部セクションの編集を承認（上部の編集完了待ち）")
