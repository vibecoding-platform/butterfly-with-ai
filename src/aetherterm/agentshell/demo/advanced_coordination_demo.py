...
class IntelligentParentAgent:
    ...
    def __init__(self):
        ...
        # コールバック登録
        self.orchestrator.register_intervention_callback(self.intelligent_intervention_handler)

    async def intelligent_intervention_handler(self, intervention: UserIntervention) -> Any:
        """インテリジェントな介入処理"""
        logger.info(f"\n[親エージェント] 介入要求を分析中...")
    ...
    async def gather_context(self, intervention: UserIntervention) -> Dict[str, Any]:
        """介入に関するコンテキストを収集"""
        context = {
    ...
    async def make_intelligent_decision(
        self, intervention: UserIntervention, context: Dict[str, Any]
    ) -> str:
        ...
        if context["conflict_history"]:
            conflict_resolution = await self.resolve_conflicts_intelligently(intervention, context)
    ...
    def is_security_risk(self, intervention: UserIntervention, context: Dict[str, Any]) -> bool:
        """セキュリティリスクをチェック"""
        risk_keywords = [
            "delete",
            "drop",
    ...
    async def resolve_conflicts_intelligently(
        ...
        if conflict_types.count(ConflictType.RESOURCE_CONFLICT) >= 2:
            ...
            self.orchestrator._coordinator.add_coordination_rule(
                rule_type="force_sequential", resource_type="database"
            )
            return "承認"  # 順次実行で承認

        return None

    def make_default_decision(self, intervention: UserIntervention, context: Dict[str, Any]) -> str:
        """デフォルトの決定ロジック"""
        # 時間帯による判断
    ...
    def extract_keywords(self, message: str) -> List[str]:
        ...
        # 重要なキーワードをフィルタ
        important_words = [
            w for w in words if len(w) > 3 and w not in ["this", "that", "with", "from", "have"]
        ]
    ...
    def find_similar_decisions(self, intervention: UserIntervention) -> List[Dict[str, Any]]:
        """過去の類似決定を検索"""
    ...
    def analyze_success_patterns(self, similar_decisions: List[Dict[str, Any]]) -> Optional[str]:
        """成功パターンを分析"""
        # 成功した決定をカウント
        success_count = {}
    ...
    def record_decision(
        self, intervention: UserIntervention, context: Dict[str, Any], decision: str
    ) -> None:
        """決定を記録"""
