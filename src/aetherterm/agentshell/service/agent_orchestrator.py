...
class AgentOrchestrator:
    ...
    async def create_child_agent(
        self, agent_type: str, task: AgentTask, pane_config: Optional[PaneConfig] = None
    ) -> ChildAgentHandle:
        """
        子エージェントを作成
        ...
        if not pane_config:
            pane_config = PaneConfig(
                ...
                agent_config={"agent_type": agent_type, "auto_start": True},
            )
        ...
        message = MessageBuilder.create_task(
            from_agent=self._agent_id, to_agent="agentserver", request=request
        )
        ...
        if response and response.message_type == MessageType.TASK_CREATE:
            ...
            handle = ChildAgentHandle(agent_id=agent_id, task_id=task.id, pane_id=pane_id)
            ...
            if self._activity_recorder:
                await self._activity_recorder.record_agent_action(
                    ...
                    metadata={"task_id": str(task.id), "pane_id": pane_id},
                )
    ...
    async def handle_child_progress(
        self, agent_id: str, progress: float, message: str, details: Optional[Dict[str, Any]] = None
    ) -> None:
        """
        子エージェントからの進捗を処理
    ...
    async def handle_intervention_request(
        self, agent_id: str, intervention: UserIntervention
    ) -> Any:
        """
        介入要求を処理
        ...
        try:
            ...
            if self._activity_recorder:
                await self._activity_recorder.record_agent_action(
                    ...
                    metadata={"intervention_type": intervention.type, "response": str(response)},
                )
    ...
    def register_progress_callback(self, callback: Callable[[str, float, str], None]) -> None:
        """進捗コールバックを登録"""
        if callback not in self._progress_callbacks:
            self._progress_callbacks.append(callback)

    def unregister_progress_callback(self, callback: Callable[[str, float, str], None]) -> None:
        """進捗コールバックを解除"""
        if callback in self._progress_callbacks:
            self._progress_callbacks.remove(callback)

    def register_intervention_callback(self, callback: Callable[[UserIntervention], Any]) -> None:
        """介入コールバックを登録"""
        if callback not in self._intervention_callbacks:
            self._intervention_callbacks.append(callback)

    def unregister_intervention_callback(self, callback: Callable[[UserIntervention], Any]) -> None:
        """介入コールバックを解除"""
        if callback in self._intervention_callbacks:
            self._intervention_callbacks.remove(callback)

    def register_completion_callback(self, callback: Callable[[str, Dict[str, Any]], None]) -> None:
        """完了コールバックを登録"""
    ...
    def _register_message_handlers(self) -> None:
        ...
        self.server_connector.register_handler(
            MessageType.PROGRESS_UPDATE, self._handle_progress_message
        )

        # 介入要求
        self.server_connector.register_handler(
            MessageType.INTERVENTION_REQUEST, self._handle_intervention_message
        )

        # タスク完了
        self.server_connector.register_handler(
            MessageType.TASK_COMPLETE, self._handle_task_complete_message
        )

        # タスク失敗
        self.server_connector.register_handler(
            MessageType.TASK_FAILED, self._handle_task_failed_message
        )
    ...
    async def _handle_progress_message(self, message: AgentMessage) -> None:
        ...
        await self.handle_child_progress(message.from_agent, progress, msg, details)
    ...
    async def _handle_intervention_message(self, message: AgentMessage) -> None:
        ...
        response = await self.handle_intervention_request(message.from_agent, intervention)
    ...
    async def create_collaborative_agents(
        ...
        for agent_config in agents:
            ...
            await self._coordinator.register_agent_task(
                agent_id=handle.agent_id, task=task, resources=agent_config.get("resources", [])
            )

        # 協調作業を設定
        assignments = await self._coordinator.coordinate_parallel_work(
            agents=agent_ids, shared_goal=shared_goal, strategy=strategy
        )

        return handles

    async def request_inter_agent_collaboration(
        self, from_agent: str, to_agent: str, request_type: str, payload: Dict[str, Any]
    ) -> UUID:
        """
        エージェント間の協力を要求
        ...
        return await self._coordinator.request_inter_agent_collaboration(
            from_agent=from_agent, to_agent=to_agent, request_type=request_type, payload=payload
        )

    async def _check_for_conflicts(self, agent_id: str, intervention: UserIntervention) -> bool:
        """
        他のエージェントとの競合をチェック
        ...
        if conflicting_agents:
            ...
            for other_agent in conflicting_agents:
                resolution = await self._coordinator.detect_and_resolve_conflicts(
                    agent_id, other_agent, {"intervention": intervention.message}
                )
