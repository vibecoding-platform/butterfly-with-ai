...
class AgentInterface(ABC):
    ...
    async def request_user_intervention(
        self,
        task_id: UUID,
    ...
    @abstractmethod
    async def _wait_for_intervention_response(
        self,
        intervention: UserIntervention
    ) -> Any:
        """介入応答を待つ（サブクラスで実装）"""
    ...
    def register_intervention_callback(
        self,
        callback: Callable[[UserIntervention], None]
    ) -> None:
        """ユーザー介入コールバックを登録"""
    ...
    def register_progress_callback(
        self,
        callback: Callable[[UUID, float, str], None]
    ) -> None:
        """進捗通知コールバックを登録"""
