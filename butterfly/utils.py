...
class ConnectionInfo(object):
    ...
    def __init__(self, environ=None, socket_remote_addr=None):
        ...
        if header_remote_addr and socket_remote_addr and header_remote_addr != socket_remote_addr:
            ...
            self.remote_addr = (
                header_remote_addr or
                socket_remote_addr or
...
class Socket(object):
    ...
    def __init__(self, socket):
        ...
        if self.user is None:
            ...
            try:
                self.user = User(
                    name=get_lsof_socket_line(self.remote_addr, self.remote_port)[1]
                )
            except Exception:
                ...
                with open("/proc/%s/status" % pid) as s:
                    for line in s.readlines():
                        if line.startswith("PPid:"):
                            with open(
                                "/proc/%s/environ" % line[len("PPid:"):].strip()
                            ) as e:
                                keyvals = e.read().split("\x00")
                                env = {}
                                for keyval in keyvals:
...
def render_motd(socket, user, uri, unsecure=False, i_hereby_declare_i_dont_want_any_security_whatsoever=False):
    """
