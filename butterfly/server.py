...
from butterfly import socket_handlers
...
def setup_config_paths(config_dict):
    ...
    if not os.path.exists(conf_file):
        try:
            ...
            print(f"Could not install default butterfly.conf: {e}")

    return config_dict

# Configure logging
# This part will be handled by the container's logging_level provider
# for logger_name in ('hypercorn.access', 'hypercorn.error', 'butterfly'):
...
def create_app(**kwargs):
    ...
    container.wire(
        modules=[
            ...
            "butterfly.terminals",
            "butterfly.utils",
            "butterfly.socket_handlers",
            "butterfly.routes",  # Wire routes module
        ]
    )
...
async def start_server(**kwargs):
    ...
    for logger_name in ("uvicorn.access", "uvicorn.error", "butterfly"):
    ...
    if not unsecure:
        if not all(map(os.path.exists, [cert % host, cert_key % host, ca])):
            log.error("Unable to find butterfly certificate for host %s", host)
            log.error(cert % host)
            log.error(cert_key % host)
            log.error(ca)
            log.error("Can't run butterfly without certificate.\n")
            log.error(
    ...
    server = uvicorn.Server(
        uvicorn.Config(
            app=app, host=host, port=port, log_level=log_level_name,
...
if __name__ == "__main__":
    # This block is now handled by scripts/butterfly
    pass
