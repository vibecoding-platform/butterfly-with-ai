...
log = getLogger("butterfly.terminal")
...
class AsyncioTerminal(BaseTerminal):
                    ...
                    if hasattr(self, "pid"):
                        try:
                            ...
                            if pid != 0:  # Process has exited
                                log.info(
                                    f"Child process {self.pid} exited with status {status}"
                                )
                                break
                        except OSError:
    ...
    async def _send_motd_direct(self):
        ...
        try:
            ...
            from butterfly.utils import render_motd
            ...
            try:
                ...
                i_hereby_declare = getattr(ApplicationContainer.config, 'i_hereby_declare_i_dont_want_any_security_whatsoever', lambda: False)()
            except:
